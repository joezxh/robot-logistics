...
当前 RCS 中存在三类"地图"概念，彼此命名重叠但数据来源分散：

| 原名称 | 入口/页面 | 底层表 | 数据内容 |
|---|---|---|---|
| 仓库地图 | `WarehouseView.vue`（`/warehouse`） | `robot_unified_maps`（`geometry_json` + `grid_json`） | 几何层（墙体/货架/区域/月台/AGV 栅格）|
| 场景地图（拓扑） | `AdminMapsView.vue`（`/admin/maps`，改名「地图配置」） | `robot_unified_maps`（`topology_json` + `robot_site_map_versions`） | 节点/边图结构 + 版本 |
| 站点地图 | `SiteMapView.vue`（`/sitemap`） | `robot_unified_maps` | 以 `map_id` 渲染几何 + 模板下拉 |
...
## 3. 迁移策略（A：新建 + 一次性迁移 + 旧表保留）

新增迁移 `migrations/007_unified_map.sql`（幂等，可重复执行）：
...
6. **旧表 `robot_topology_shell`、`robot_site_maps` 不删除**，文件末尾注释标记：
   ```sql
   -- DEPRECATED: robot_topology_shell, robot_site_maps; remove in 008_drop_legacy.sql
   ```
7. 后续 `008_drop_legacy.sql` 在验证通过后执行（本阶段不执行）。
...
- `SiteMapView` 与 `AdminMapsView` 均调 `GET /maps/{id}`，分别消费 `geometry` / `topology` 字段。
...
### 6.3 视图
- `SiteMapView.vue`：下拉选 `map_id`；`loadByMap`。
- `AdminMapsView.vue`：标题改为「地图配置」；读 `topology` 字段编辑。
- `WarehouseView.vue`：调 `getMap(MAP_ID).geometry`。
...
- **菜单/侧边栏（`sys_seed.py`）**：
  - 站点地图（保留，`/sitemap`，归入「仓储中心」分组）
  - 场景地图 → **地图配置**（`/admin/maps`）
  - 仓库视图（保留，`/warehouse`）
...
- `SiteMapView.spec.ts` / `AdminMapsView.spec.ts`：改调统一接口。
- `types/siteGrid.spec.ts`：grid 序列化对齐。

### 7.3 验证
- `pnpm run test`（前端单测）、`pytest`（后端单测）全绿。
- `tsc --noEmit` + `vite build` 通过。
- Playwright 真实浏览器走查：选模板 → 站点地图渲染 → 切「地图配置」编辑节点 → 仓库视图布局；确认无 Vue 报错、接口无 500。
...
| 命名混淆 | 菜单「场景地图→地图配置」+ 产品标题「RCS 控制台」，消除「站点地图」多重含义 |
