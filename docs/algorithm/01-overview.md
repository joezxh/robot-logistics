...
>
> **实现状态**：本章描述目标架构设计。当前已实现四子项目 Monorepo 架构（`shared/`、`rcs/`、`simulation/`、`robot-app/`）+ `vla-training/` 训练管线，Phase 1 双臂 AGV 装卸机器人 + Phase 2 感知与导航 Task 1-6 已完成，308 tests 全部通过。详见 [ARCHITECTURE.md](../ARCHITECTURE.md)。
...
| 模块名称 | 位置 | 功能职责 | 实时性要求 | 实现状态 |
|---------|------|---------|-----------|----------|
| **运动规划器** | 边缘 | 全局路径+局部优化+轨迹插补 | <2s规划 | ✅ 设计完成 |
| **BaseExecutor** | robot-app | Nav2 NavigateToPose action client | 秒级 | ✅ Phase 2 已实现 |
| **PointCloudProcessor** | robot-app | 7 步点云处理管线 | <100ms | ✅ Phase 2 已实现 |
| **合成传感器** | simulation | PointCloudGenerator + LaserScanGenerator | 10Hz | ✅ Phase 2 已实现 |
| **目标检测** | 边缘 | 通用物体检测（闭集+开集） | <50ms | 🔲 设计完成 |
| **6-DoF姿态估计** | 云端 | 物体精确位姿 | <80ms | 🔲 设计完成 |
| **抓取姿态规划** | 云端 | 最优抓取计算 | <100ms | 🔲 设计完成 |
| **任务调度器** | 云端 | 批次优化 | 秒级 | ✅ TaskCoordinator 已实现 |
| **TaskCoordinator** | robot-app | 9 阶段 FSM + ABORTING | 实时 | ✅ Phase 1 已实现 |
