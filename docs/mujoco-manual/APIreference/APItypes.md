> [中文](APItypes_CN.md) | English

# Types

MuJoCo defines a large number of types:

  * Four [primitive types](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#typrimitive): [mjtNum](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjtnum), [mjtByte](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjtbyte), [mjtBool](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjtbool), and [mjtSize](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjtsize).

  * [C enum types](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyenums) used to define categorical values. These can be classified as:

    * Enums used in [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tymodelenums).

    * Enums used in [mjData](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tydataenums).

    * Enums for abstract [visualization](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyvisenums).

    * Enums used by the [classic renderer](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyrenderenums).

    * Enums used by the [filament renderer](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyfilamentrenderenums).

    * Enums used by the [mjUI](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyuienums) user interface package.

    * Enums used by [engine plugins](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#typluginenums).

    * Enums used for [procedural model manipulation](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyspecenums).

Note that the API does not use these enum types directly. Instead it uses ints, and the documentation/comments state that certain ints correspond to certain enum types. This is because we want the API to be compiler-independent, and the C standard does not dictate how many bytes must be used to represent an enum type. Nevertheless, for improved readability, we recommend using these types when calling API functions which take them as arguments.

  * [C struct types](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tystructure). These can be classified as:

    * Main structs:

      * [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel).

      * [mjOption](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjoption) (embedded in [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel)).

      * [mjData](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjdata).

    * [Auxiliary struct types](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyauxstructure), also used by the engine.

    * Structs for collecting [simulation statistics](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tystatstructure).

    * Structs for [abstract visualization](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyvisstructure).

    * Structs used by the [classic renderer](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyrenderstructure).

    * Structs used by the [filament renderer](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyfilamentrenderstructure).

    * Structs used by the [UI framework](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyuistructure).

    * Structs used for [procedural model manipulation](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyspecstructure).

    * Structs used by [engine plugins](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#typluginstructure).

  * Several [function types](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tyfunction) for user-defined callbacks.

  * [Notes](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tynotes) regarding specific data structures that require detailed description.
...
The three types below are defined in [mjtype.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjtype.h).
...
Note that changing `mjUSESINGLE` in `mjtype.h` will not change how the library was compiled, and instead will result in numerous link errors. In general, the header files distributed with precompiled MuJoCo should never be changed by the user.
...
The enums below are defined in [mjtype.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjtype.h).

#### mjtDisableBit

Constants which are powers of 2. They are used as bitmasks for the field `disableflags` of [mjOption](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjoption). At runtime this field is `m->opt.disableflags`. The number of these constants is given by `mjNDISABLE` which is also the length of the global string array [mjDISABLESTRING](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjdisablestring) with text descriptions of these flags.
...
Constants which are powers of 2. They are used as bitmasks for the field `enableflags` of [mjOption](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjoption). At runtime this field is `m->opt.enableflags`. The number of these constants is given by `mjNENABLE` which is also the length of the global string array [mjENABLESTRING](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjenablestring) with text descriptions of these flags.
...
The type of a light source describing how its position, orientation and other properties will interact with the objects in the scene. These values are used in `m->light_type`.
...
MuJoCo object types. These are used, for example, in the support functions [mj_name2id](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-name2id) and [mj_id2name](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-id2name) to convert between object names and integer ids.
...
These are the compute stages for the skipstage parameters of [mj_forwardSkip](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-forwardskip) and [mj_inverseSkip](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-inverseskip).
...
Types of frame alignment of elements with their parent bodies. Used as shortcuts during [mj_kinematics](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-kinematics) in the last argument to [mj_local2Global](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-local2global).
...
Sleep policy associated with a tree. The compiler automatically chooses between `NEVER` and `ALLOWED`, but the user can override this choice. Only the user can set the `INIT` policy (initialized as asleep).
...
State component elements as integer bitflags and several convenient combinations of these flags. Used by [mj_getState](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-getstate), [mj_setState](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-setstate) and [mj_stateSize](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-statesize).
...
Timer types. The number of timer types is given by `mjNTIMER` which is also the length of the array `mjData.timer`, as well as the length of the string array [mjTIMERSTRING](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjtimerstring) with timer names.
...
Topic identifiers for informational messages. Used with [mju_info](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mju-info) for topic-based filtering. Topic 0 (`mjTOPIC_NONE`) always passes through the default handler’s filter. Other topics must be enabled in the [mjLogConfig](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjlogconfig) bitmask. Since topics are 1-indexed, the bitmask for topic `t` is `(1 << (t - 1))`.
...
The enums below are defined in [mjvisualize.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjvisualize.h).

#### mjtCatBit

These are the available categories of geoms in the abstract visualizer. The bitmask can be used in the function [mjr_render](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mjr-render) to specify which categories should be rendered.
...
These are the mouse actions that the abstract visualizer recognizes. It is up to the user to intercept mouse events and translate them into these actions, as illustrated in [simulate.cc](https://mujoco.readthedocs.io/en/stable/APIreference/programming/samples.md#sasimulate).
...
The enums below are defined in [mjrender.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjrender.h).

#### mjtGridPos

These are the possible grid positions for text overlays. They are used as an argument to the function [mjr_overlay](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mjr-overlay).
...
These are the possible framebuffers. They are used as an argument to the function [mjr_setBuffer](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mjr-setbuffer).
...
These are the depth mapping options. They are used as a value for the `readPixelDepth` attribute of the [mjrContext](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrcontext) struct, to control how the depth returned by [mjr_readPixels](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mjr-readpixels) is mapped from `znear` to `zfar`.
...
The enums below are defined in [mjrfilament.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjrfilament.h).
...
The enums below are defined in [mjui.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjui.h).
...
The enums below are defined in [mjspec.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjspec.h).
...
Compiler timing categories, used in [mjs_getTimer](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mjs-gettimer). Top-level timers (`TOTAL`, `ASSETS`) measure wall-clock time. Asset sub-timers measure CPU time summed across all assets; with multi-threaded compilation their sum can exceed the `ASSETS` wall-clock time.
...
The enums below are defined in [mjplugin.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjplugin.h). See [Engine plugins](https://mujoco.readthedocs.io/en/stable/APIreference/programming/extension.md#explugin) for details.
...
The three central struct types for physics simulation are [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel), [mjOption](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjoption) (embedded in [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel)) and [mjData](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjdata). An introductory discussion of these structures can be found in the [Overview](https://mujoco.readthedocs.io/en/stable/APIreference/overview.md#modelanddata).

### mjModel

This is the main data structure holding the MuJoCo model. It is treated as constant by the simulator. Some specific details regarding datastructures in [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel) can be found below in [Notes](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#tynotes).
...
This is the data structure with simulation options. It corresponds to the MJCF element [option](https://mujoco.readthedocs.io/en/stable/APIreference/XMLreference.md#option). One instance of it is embedded in mjModel.
...
These struct types are used in the engine and their names are prefixed with `mj`. [mjVisual](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjvisual) and [mjStatistic](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjstatistic) are embedded in [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel), [mjContact](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjcontact) is embedded in [mjData](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjdata), and [mjVFS](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjvfs) is a library-level struct used for loading assets.

#### mjVisual

This is the data structure with abstract visualization options. It corresponds to the MJCF element [visual](https://mujoco.readthedocs.io/en/stable/APIreference/XMLreference.md#visual). One instance of it is embedded in mjModel.
...
This is the data structure with model statistics precomputed by the compiler or set by the user. It corresponds to the MJCF element [statistic](https://mujoco.readthedocs.io/en/stable/APIreference/XMLreference.md#statistic). One instance of it is embedded in mjModel.
...
A resource is an abstraction of a file in a filesystem. The name field is the unique name of the resource while the other fields are populated by a [resource provider](https://mujoco.readthedocs.io/en/stable/APIreference/programming/extension.md#exprovider).
...
Options for configuring the automatic [actuator length-range computation](https://mujoco.readthedocs.io/en/stable/APIreference/modeling.md#clengthrange).
...
Asset cache used by the compiler to avoid repeated slow recompilation. See [Asset cache](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#assetcache).
...
Structured log message passed to [mjfLogHandler](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjfloghandler) callbacks. Contains the severity level, optional topic for info messages, a one-line subject, an optional multi-line body, and optional source location (function name, file name, line number).
...
Configuration for the default log handler. Controls whether messages are printed to the console and/or written to a log file (default: `MUJOCO_LOG.TXT`). The `logto_file` field enables file logging, while `logfile` specifies the file path. The `topics` field is a bitmask of [mjtLogTopic](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjtlogtopic) values: bit `(topic - 1)` enables that topic. Topic 0 (`mjTOPIC_NONE`) always passes through.
...
These structs are all embedded in [mjData](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjdata), and collect simulation-related statistics.
...
This structure contains everything needed to render a 2D plot in OpenGL. The buffers for line points etc. are preallocated, and the user has to populate them before calling the function [mjr_figure](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mjr-figure) with this data structure as an argument.
...
The names of these struct types are prefixed with `mjrf`. They are defined in [mjrfilament.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjrfilament.h).

There are seven key types defined by this API: [mjrfContext](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfcontext), [mjrfTexture](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrftexture), [mjrfMesh](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfmesh), [mjrfLight](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrflight), [mjrfRenderable](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfrenderable), [mjrfScene](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfscene), and [mjrfRenderTarget](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfrendertarget).

Each object is created using a `create` function and destroyed using a `destroy` function, e.g. `mjrf_createTexture` and `mjrf_destroyTexture`. All objects require a [Context](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfcontext) in order to be created (with the exception of the [Context](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfcontext) object itself). Additionally, the `create` functions accept a pointer to a configuration struct (e.g. `mjrTextureConfig`) which describes the parameters for the object to be created. Each of these structs has a corresponding `default` function (e.g. `mjrf_defaultTextureConfig`) which can be used to initialize the struct to default values. The default values are assumed to be `0` or `NULL` unless otherwise specified.

#### mjrfContext

The Context is the main entry point for the filament rendering library. It manages all the core filament objects that are responsible for the rendering of an image. All other objects (e.g. Textures, Meshes, Scenes, etc.) need a Context in order to be created.

Otherwise, the main function to use with the Context is [mjrf_render()](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mjrf-render) which performs the actual rendering of an image.

Filament uses a separate thread for rendering. However, despite that, this API is not thread-safe; calls are expected to be made from a single thread. Due to the asynchronous nature of filament, some APIs provide handles or callbacks to signal when an operation is complete. (Note: for WASM builds, filament does not use a separate thread.)

There are two key differences between the [mjrfContext](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfcontext) and the classic [mjrContext](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrcontext). Firstly, the filament context will manage the underlying graphics context itself. This means users do not need to initialize EGL or similar libraries beforehand. Secondly, the filament context is independent of a MuJoCo model. That means you can use a single [mjrfContext](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfcontext) instance to render images for multiple models.

#### mjrfContextConfig

Parameters for creating [filament graphics context](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfcontext).
...
A texture is a 2D or 3D (cubemap) image that adds visual detail to a rendered model, such as color or bumpiness, without increasing geometric complexity. A texture is simply a memory buffer holds pixel data, as well as metadata such as the dimensions of the image or the format of the pixels (e.g. 8-bit RGB).

#### mjrfTextureConfig

Parameters for creating a [texture](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrftexture).
...
Binary data payload for a [texture](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrftexture).
...
Filament does not directly support normals. Instead, it encodes the normal, tangent, and bitangent into a 4-component quaternion describing the “orientation” of the vertex. Ideally, you should preprocess your assets to generate this data offline, but we will compute it on the fly if needed (at a performance cost).

Vertex data may or may not be interleaved. Interleaved data assumes that the attributes are packed in the order specified in the attributes array, with no padding in-between. Additionally, the `data` pointer for each attribute is assumed to point to the first element of that type. For non-interleaved data, each attribute is assumed to be stored in a separate array.

Additionally, the bounds of the mesh should be computed in order to allow the filament renderer to perform frustum-based culling. Alternatively, the bounds can be computed at runtime (though there is a small performance cost). If no bounds are provided (or calculated), then frustum culling will not be performed.

#### mjrfMeshConfig

Parameters for creating a [mesh](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfmesh).
...
Binary data for a [mesh](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfmesh).
...
A Scene is a collection of [Lights](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrflight) and [Renderables](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfrenderable) that describes what is to be rendered.

#### mjrfSceneParams

Parameters for creating a [scene](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfscene).

#### mjrfLight

A light is a source of illumination. (Without lights, a rendered image will be completely black.) There are several different types of lights such as directional, spot, point, and image lights.

The primary light in a scene is the image light (also sometimes known as the environment light). This is a light that “surrounds” the entire scene and is defined as a 3D texture. Each “pixel” of the cubemap is interpreted as the color of projected into the scene from a particular direction.

The texture used for image-based lighting can be generated using filament’s `cmgen` tool. The tool should be configured to output a KTX file from your source image. This tool calculates additional data (i.e. the spherical harmonics) and encodes that information into the KTX file.

Directional lights are the next most common type of light and is usually used to simulate the sun; a uniformly colored light that is emitted in a single direction.

Filament only supports a single image and directional light. You can define as many point or spot lights as you want. Each light source (except image based lights) may or may not cast shadows. Each shadow-casting light incurs a performance cost.

#### mjrfLightParams

Parameters for creating a [light](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrflight).
...
A renderable is a single object that is to be drawn. It is defined as a combination of a [mjrfMesh](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfmesh) (i.e. the shape or surface geometry, as described above) and a [mjrfMaterial](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfmaterial) (i.e. a description of how the surface interacts with lights to product the final visual appearance).

#### mjrfRenderableParams

Parameters for creating a [renderable](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfrenderable).
...
Materials describe the properties of the surface of a renderable, effectively dictating how the surface interacts with lights to produce a final pixel color in the output image. Different lighting models will be applied to the surface depending on the values of the material properties. There there are three lighting models currently supported:

1\. Metallic-roughness (PBR): this is the preferred model for rendering models based standard metallic-roughness workflows.

2\. Specular-glossiness (non-PBR): this is a legacy model designed to be compatible with classic [mjr](https://mujoco.readthedocs.io/en/stable/APIreference/programming/visualization.md#rendering) renderer, though it is not 100% identical.

3\. Unlit: this model ignores lighting and is used for rendering UX or decorative elements like contact forces and labels.
...
A RenderTarget is a memory buffer that holds the results of a rendering operation. (This is an alternative to rendering directly to the screen.)

#### mjrfRenderTargetConfig

Parameters for creating a [render target](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjrfrendertarget).
...
For a high-level description of the UI framework, see [User Interface](https://mujoco.readthedocs.io/en/stable/APIreference/programming/ui.md#ui). The names of these struct types are prefixed with `mjui`, except for the main [mjUI](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjui) struct itself.
...
The structs below are defined in [mjspec.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjspec.h) and, with the exception of the top level [mjSpec](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjspec) struct, begin with the `mjs` prefix. For more details, see the [Model Editing](https://mujoco.readthedocs.io/en/stable/APIreference/programming/modeledit.md) chapter.
...
Special type corresponding to any element. This struct is the first member of all other elements; in the low-level C++ implementation, it is not included as a member but via class inheritance. Inclusion via inheritance allows the compiler to `static_cast` an `mjsElement` to the correct C++ object class. Unlike all other attributes of the structs below, which are user-settable by design, modifying the contents of an `mjsElement` is not allowed and leads to undefined behavior.
...
C handles for C++ strings and vector types. When using from C, use the provided [getters](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#attributegetters) and [setters](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#attributesetters).
...
The names of these struct types are prefixed with `mjp`. See [Engine plugins](https://mujoco.readthedocs.io/en/stable/APIreference/programming/extension.md#explugin) for more details.
...
Data structure used by the [Signed Distance Functions](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#signeddistancefunction) API for computing distances and gradients between SDF geoms.
...
This data structure contains the definition of a [resource provider](https://mujoco.readthedocs.io/en/stable/APIreference/programming/extension.md#exprovider). It contains a set of callbacks used for opening and reading resources.
...
This data structure defines a decoder. It contains a set of callbacks used for decoding [mjResource](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjresource) into [mjSpec](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjspec).
...
This data structure defines an encoder. It contains a set of callbacks used for encoding of [mjSpec](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjspec) and [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel) into [mjResource](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjresource).
...
MuJoCo callbacks have corresponding function types. They are defined in [mjdata.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjdata.h) and in [mjui.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjui.h). The actual callback functions are documented in the [globals](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md) page.

### Physics Callbacks

These function types are used by [physics callbacks](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#glphysics).
...
This is the function type of the callbacks [mjcb_passive](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjcb-passive) and [mjcb_control](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjcb-control).
...
This is the function type of the callback [mjcb_contactfilter](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjcb-contactfilter). The return value is 1: discard, 0: proceed with collision check.
...
This is the function type of the callback [mjcb_sensor](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjcb-sensor).
...
This is the function type of the callback [mjcb_time](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjcb-time).
...
This is the function type of the callbacks [mjcb_act_dyn](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjcb-act-dyn), [mjcb_act_gain](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjcb-act-gain) and [mjcb_act_bias](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjcb-act-bias).
...
This is the function type of the callbacks in the collision table [mjCOLLISIONFUNC](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIglobals.md#mjcollisionfunc).
...
This is the function type of the log handler callback installed via [mju_setLogHandler](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mju-setloghandler). The handler receives all errors, warnings and informational messages as structured [mjLogMessage](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjlogmessage) data. It must be thread-safe.

It must not call [mju_error](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mju-error) from within the callback.
...
These callbacks are used by [resource providers](https://mujoco.readthedocs.io/en/stable/APIreference/programming/extension.md#exprovider).
...
This callback is for opening a resource; returns zero on failure. Note that if this callback returns zero, the `close` callback will not be called. Therefore, the `open` callback is responsible for cleaning up any allocated memory or resources before returning zero to avoid memory leaks.
...
This callback is given an opened resource, and is responsible for decoding it into a [mjSpec](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjspec). Ownership of the resource and the returned spec is responsibility of the caller. When decoding fails, the callback should return NULL.
...
This callback is given an opened resource, and is responsible for returning true if the resource can be decoded by the [mjpDecoder](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjpdecoder).
...
This callback populates the [mjResource](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjresource) `data` member with bytes representing the given spec in the format associated with the owning plugin. This may be called with the associated compiled [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel).
...
[mjData](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjdata) contains two arrays with the `c` prefix, which are used for internal calculations: `cdof` and `cinert`, both computed by [mj_comPos](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-compos). The `c` prefix means that quantities are with respect to the “c-frame”, a frame at the center-of-mass of the local kinematic subtree (`mjData.subtree_com`), oriented like the world frame. This choice increases the precision of kinematic computations for mechanisms that are distant from the global origin.

`cdof`:
    

These 6D motion vectors (3 rotation, 3 translation) describe the instantaneous axis of a degree-of-freedom and are used by all Jacobian functions. The minimal computation required for analytic Jacobians is [mj_kinematics](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-kinematics) followed by [mj_comPos](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-compos).

`cinert`:
    

These 10-vectors describe the inertial properties of a body in the c-frame and are used by the Composite Rigid Body algorithm ([mj_crb](https://mujoco.readthedocs.io/en/stable/APIreference/APIreference/APIfunctions.md#mj-crb)). The 10 numbers are packed arrays of lengths (6, 3, 1) with semantics:
...
The convex hull descriptors are stored in [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel):
...
If mesh `N` has a convex hull stored in [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel) (which is optional), then `m->mesh_graphadr[N]` is the offset of mesh `N`’s convex hull data in `m->mesh_graph`. The convex hull data for each mesh is a record with the following format:
