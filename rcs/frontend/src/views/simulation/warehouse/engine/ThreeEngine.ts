...
export class ThreeEngine {
  ...
  buildWalls(shell: ShellBlueprint): void {
    ...
    // `B` was referenced below but never declared — under the simulation's looser
    // tsconfig this slipped through as an implicit any; RCS's strict config
    // surfaces it. It is the shell's footprint.
    const B = shell.bounds
    ...
    if (!this._shellFramed) {
      ...
      // Scene.fog is typed as FogBase, but only THREE.Fog carries `far`.
      const fog = this.scene.fog as THREE.Fog | null
      if (fog) fog.far = Math.max(fog.far, Math.max(B.w, B.d) * 2.4)
    }
  }
  ...
  // The builders below are intentionally unimplemented stubs (each zone/facility
  // type is meant to land in its own module). Parameters are prefixed with `_`
  // so the stricter `noUnusedParameters` check passes while the public
  // signatures stay stable for callers.

  buildAsrsZone(_zone: Zone): void {
    // Placeholder - implemented in zones/asrs.ts
  }

  buildHighRackZone(_zone: Zone): void {
    // Placeholder - implemented in zones/highRack.ts
  }

  buildMezzanineZone(_zone: Zone): void {
    // Placeholder - implemented in zones/mezzanine.ts
  }

  buildTempZone(_zone: Zone): void {
    // Placeholder - implemented in zones/temp.ts
  }

  buildTempBaggedZone(_zone: Zone): void {
    // Placeholder - implemented in zones/tempBagged.ts
  }

  buildReturnsZone(_zone: Zone): void {
    // Placeholder - implemented in zones/returns.ts
  }

  buildFacilities(_facilities: any[], _shell: ShellBlueprint | undefined): void {
    // Placeholder - implemented in facilities/index.ts
  }

  buildDocks(_docks: any[], _shell: ShellBlueprint | undefined): void {
    // Placeholder - implemented in docks/index.ts
  }

  buildVehicles(_vehicles: any[], _shell: ShellBlueprint | undefined): void {
    // Placeholder - implemented in vehicles/index.ts
  }

  // `cwEl` is the caller's container, but picking/scrolling are bound to the
  // engine's own canvas, so it goes unused — prefixed rather than dropped to
  // keep the call sites unchanged.
  bindMouse(_cwEl: HTMLElement, onHover: HoverCallback, onClick: ClickCallback): void {
