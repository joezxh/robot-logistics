...
import type { ShellOrigin } from '../api/shell'
...
export const useWarehouseStore = defineStore('warehouse', () => {
  ...
  /** Which backend supplied `floorFull.shell` (see ../api/shell.ts). */
  const shellOrigin = ref<ShellOrigin>('simulation')
  /** Which backend supplied the inventory layer (slots / items / AGV / tasks). */
  const inventoryOrigin = ref<ShellOrigin>('simulation')
  ...
  /**
   * Which backend supplied the current geometry — surfaced in the UI so it is
   * obvious whether RCS-authored edits are live.
   */
  function setShellOrigin(o: ShellOrigin) {
    shellOrigin.value = o
  }

  function setInventoryOrigin(o: ShellOrigin) {
    inventoryOrigin.value = o
  }

  function setLogisticsTasks(t: LogisticsTask[]) {
    logisticsTasks.value = t
  }

  function setLogisticsStats(s: LogisticsStats) {
    logisticsStats.value = s
  }
