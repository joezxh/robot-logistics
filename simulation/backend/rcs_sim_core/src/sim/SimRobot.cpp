...
SimRobot::SimRobot(std::shared_ptr<Sim> sim, SimRobotConfig cfg,
                   bool register_convergence_callback)
    : SimRobot(sim,
               std::make_shared<common::MjIK>(sim->m, sim->d, cfg.joints,
                                              cfg.attachment_site, cfg.base),
               cfg, register_convergence_callback) {}
