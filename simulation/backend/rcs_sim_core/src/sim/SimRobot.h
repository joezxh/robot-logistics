...
class SimRobot : public common::Robot {
 public:
  ...
  // Convenience overload: builds a MuJoCo-native IK (MjIK) internally, so no
  // external kinematics library (pinocchio) is required.
  SimRobot(std::shared_ptr<rcs::sim::Sim> sim, SimRobotConfig cfg,
           bool register_convergence_callback = true);
  ~SimRobot() override;
