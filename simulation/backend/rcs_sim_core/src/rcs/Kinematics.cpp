// pinocchio-based IK implementation. Compiled only when pinocchio is available;
// otherwise this translation unit is empty (the MuJoCo-native MjIK is used).
...
Pin::Pin(const std::string& path, const std::string& frame_id, bool urdf)
  ...
  if (FRAME_ID == -1) {
    throw std::runtime_error(frame_id +
                             " frame id could not be found in the provided model");
  }
}
